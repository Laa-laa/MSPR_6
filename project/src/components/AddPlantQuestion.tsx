...
const AddPlantQuestion: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const response = await axios.post('http://127.0.0.1:8000/api/plantsQuestions', {
        title: title,
        content: content,
      });
    ...
    <div className="addPlantQuestionHeader mb-8">
      ...
      <form className="addPlantQuestionForm mx-auto" onSubmit={handleSubmit}>
          <div className="mb-4">
              ...
              <input type="text" id="title" name="title" className="border border-gray-300 rounded-md p-2 w-full" required />
          </div>
          <div className="mb-4">
              <label htmlFor="description" className="block">Description :</label>
              <textarea id="description" name="description" rows={4} className="border border-gray-300 rounded-md p-2 w-full" required></textarea>
          </div>
          <div className="mb-4">
